...
export class JobCategoriesPage extends BasePage {
  ...
  constructor(page: Page) {
    ...
    this.addHeader = page.locator('h6:has-text("Add Job Category")');
    this.editHeader = page.locator('h6:has-text("Edit Job Category")');
  ...
  get url(): string {
    return ROUTES.adminJobCategories;
  }
  ...
  async clickAdd(): Promise<void> {
    logger.info('Clicking Add button on job categories page');
  ...
  async clickEdit(rowIndex: number): Promise<void> {
    logger.info(`Editing job category at row ${rowIndex}`);
